...
//        Edge[] e = edges.toArray(new Edge[0]);
    ...
    public int getShortestDistance(final Edge route) {
        assert (route != null) : "The route is empty";

        int d = getShortestDistance(route.getFrom(), route.getTo());
        route.setWeight(d);
    ...
    public List<Node> getShortestPath(final Edge route) {
        assert (route != null) : "The route is empty";

        return getShortestPath(route.getFrom(), route.getTo());
    }
